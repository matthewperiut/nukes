...
public class GenericNuke extends Block {

    public static final BooleanProperty UNSTABLE;
    private final TntBuilder creator;

    public GenericNuke(AbstractBlock.Settings settings, final TntBuilder creator) {
        super(settings);
        this.setDefaultState(this.getDefaultState().with((Property) GenericNuke.UNSTABLE, (Comparable) false));
    ...
    public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        if (!world.isClient() && !player.isCreative() && (boolean) state.get((Property) GenericNuke.UNSTABLE)) {
            this.primeTnt(world, pos);
        }
    ...
    public void onDestroyedByExplosion(World world, BlockPos pos, Explosion explosion) {
        if (!world.isClient) {
            final GenericNukeEntity tntEntity = this.creator.create(world, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, explosion.getCausingEntity());
    ...
    public void primeTnt(World world, BlockPos pos, @Nullable LivingEntity livingEntity) {
        if (!world.isClient) {
            final GenericNukeEntity tntEntity = this.creator.create(world, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, livingEntity);
            world.spawnEntity(tntEntity);
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(GenericNuke.UNSTABLE);
    }
